		...
		<table class="reportview-table report-table table table-bordered table-hover table-striped table-condensed" id="reportview_table">
			...
			<tbody>
				...
				<? foreach ($report["total_daily_report_list"] as $total_daily_report) { ?>
					<tr>
						<!-- サマリ項目は空 -->
						<td class="summary-unit">日別</td>
						<? for ($i = 0; $i < count($summary_elem_list); $i++) { ?>
							<td class="summary-unit"><?= isset($total_daily_report[$i]) ? $total_daily_report[$i] : "--"; ?></td>
						<? } ?>
						<!-- 出力項目 -->
						<? for ($j = $i; $j < count($total_daily_report); $j++) { ?>
							<td class="text-right"><?= isset($total_daily_report[$j]) ? $total_daily_report[$j] : "--"; ?></td>
						<? } ?>
					</tr>
				<? } ?>
				<!-- サマリ実績出力 -->
