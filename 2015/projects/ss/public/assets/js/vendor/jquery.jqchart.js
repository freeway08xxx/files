		...
		jQuery.extend( $.fn.jQchart,{
			...
			init :	function (op){ 
				...
				this.op=jQuery.extend({
					...
					//canvas property //縺ゅ→縺ｧline繧・ｻｸ蛻･縺ｫconfig險ｭ螳壼庄閭ｽ縺ｫ縺吶ｋ
					...
					//繝輔か繝ｳ繝医し繧､繧ｺ
					scaleFontSize	: op.config.scaleFontSize || op.fontSize ||	9,
					dataFontSize	: op.config.dataFontSize	|| op.fontSize ||10,
					
					//繝√Ε繝ｼ繝磯伜沺縺ｮ繝代ョ繧｣繝ｳ繧ｰ
					...
					//Title
					title		: op.config.title		|| '<a href="/redirect/cgi/r2.cgi?gs=javascript&type=c&id=CU20000123J&url=http://jsgt.org/mt/archives/01/001827.html" target="_blank">jquery.jQchart</a>',
				...
				this.op.grid	= {
						col:(this.op.labelX)?
							...
							this.op.data[0].length+this.op.xGapPaddingR||50,//證ｫ螳・
				...
				if (this.canvas.getContext){
					...
					function draggingon(oj){
						...
						$(oj).draggable({opacity:0.5 })
								...
								/*.append($('<div id="jQchart-close-'+oj.id+'" 									style="font-size:0.9em;									width:300px;									color:#333;									padding:0px">[X]-close</div>').one('click',function(){
										$(_cbox).replaceWith('<canvas id="'+its.canvas.id+'"></canvas>')
									}))
								*/
								.append('<div id="jQchart-drgmsg-M-'+oj.id+'" class="jQchart-drgmsg-M">draggable-繝峨Λ繝・げ縺ｧ縺阪∪縺・/div>')
			...
			wrtYscale	: function(op){
				...
				for (var i = 0, currP=0; i <= op.grid.row;i++) {
					...
					currP = Math.round((currP*100))/100;//荳ｸ繧・
			...
			wrtGraph : function(op){	//200:y=700:100
				...
				$.each(op.data,function(index,value){
					...
					var strokeStyle=op.line_strokeStyle[index]||'#777';//縺ｨ繧翫≠縺医★7color
					its.ctx.strokeStyle=strokeStyle;//謚倥ｌ邱夊牡 縺ゅ→縺ｧoption謖・ｮ壹↓
