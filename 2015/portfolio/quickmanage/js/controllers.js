/* Controllers */
var controllers = angular.module('quickManage.controllers', [])

/**
 * 「レポート」画面
 */
controllers.controller('QuickManageReportCtrl', ['$scope', '$routeParams', '$http',
	...
	$scope.settings = {
		...
		actions: {
			changeTab :  function (tab) {
				...
				window.location = '/sem/new/quickmanage/#/report?tab='+tab;
			}
		}
	};
 ...
 *  「目標設定」画面
 */
controllers.controller("QuickManageAimCtrl", ['$scope', '$routeParams',
 ...
 *  「値引設定」画面
 */
controllers.controller("QuickManageDiscountCtrl", ['$scope', '$routeParams',
	function ($scope, $routeParams) {
 ...
 * レポート作成画面　フォーム部分
 */
controllers.controller("ReportFormCtrl", ['$scope', '$modal','quickManageRestApi','paramsFactoryService','dataSharedService','utilFactoryService',
	function ($scope, $modal,quickManageRestApi,paramsFactoryService,dataSharedService,utilFactoryService) {
	$.extend($scope.params, {
		report_type: 'summary',
		position_id: '4',
		summary_type: 'all',
	...
	$scope.submit = function () {
		...
		var data = paramsFactoryService.getParams($scope,form.serialize());
		if($scope.params.export_type==='export'){
			quickManageRestApi.getExportdata(data).then(function() {
				$scope.msg = 'レポートの作成を開始しました。';
				$scope.is_processing  = false;
			});
			...
			quickManageRestApi.getReportdata(data).then(function(res) {
				//console.log(res);
				$scope.is_processing  = false;
				if(_.isEmpty(res.models)){
					$scope.msg = '実績データがありません。';
					return false;
				}
				if(!res.error) {
					$scope.msg = '';
 ...
 */
controllers.controller("ReportTemplateCtrl", ['$scope', '$http', '$modal',
 ...
 */
controllers.controller('TemplateInsertCtrl', ['$scope', '$modalInstance', '$http',
 ...
 * ※ $modalInstance.close したいのでコントローラー分けてます
 */
controllers.controller('TemplateEditCtrl', ['$scope', '$modalInstance', '$http',
	function ($scope, $modalInstance, $http) {
 ...
 */
controllers.controller("RegistFormCtrl", ['$scope', function ($scope) {
	$scope.csv = {
		name: 'test.csv',
		data: []
	};
