...
class Controller_Axis_Report extends Controller_Axis_Base {
	...
	public function post_templatelist() {
		...
		global $report_format_list, $report_type_list, $summary_type_list, $report_term_list;
		...
		if (Input::is_ajax()) {
			...
			foreach ($DB_template_list as $key => $value) {
				...
				$report_format  = array(
					'value' => $value['report_format'],
					'label' => $report_format_list[$value['report_format']]
				);
				$report_term  = array(
					'value' => $value['report_term'],
					'label' => $report_term_list[$value['report_format']][$value['report_term']]
				);
				$report_type = array(
	...
	public function post_instemplate() {
		...
		if (Input::is_ajax()) {
			...
			$form = Input::json();
			$values = [
				'client_id'          => $form['ssClient'],
				'category_genre_id'  => $form['ssCategoryGenre'],
				...
				'media_cost'         => $form['media_cost'],
				'option_list'        => serialize($form['option_list']),
				'ext_cv_list'        => serialize($form['ext_cv_list']),
				'filter_list'        => serialize($form['filter_list']),
				'export_format'      => $form['export_format'],
				'report_name'        => $form['report_name'],
				'template_memo'      => $form['template_memo'],
				'send_mail_flg'      => $form['send_mail_flg'],
			];

			$account_values_list = [];
			foreach ($form['ssAccount'] as $tmp) {
				$account_values = [
					'client_id'  => $form['ssClient'],
			...
			## テンプレート登録・更新
			$ret = \Model_Data_Axis_Template::setting_template($form['id'], $values, $account_values_list);
